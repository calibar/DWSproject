...
export class MapPage {
  ...
  ionViewDidLoad() {
    ...
    /*var position = new google.maps.LatLng(51.132854,-106.631401); */
  ...
  showMap(){
    ...
    this.loadCouverts();
     /* navigator.geolocation.getCurrentPosition(function(position) {
      ...
      if (myLat&&myLon){
        console.log("get it")
        location=new google.maps.LatLng(myLat,myLon);
      }
      ...
      this.loader.present();
      this.geolocation.getCurrentPosition().then((resp) => {
        this.couverinfo.Lat=resp.coords.latitude// resp.coords.latitude
        this.couverinfo.Lon=resp.coords.longitude// resp.coords.longitude
        this.loader.onDidDismiss(()=>{
          console.log("Into your location")
        })
        this.loader.dismiss();
        location=new google.maps.LatLng(this.couverinfo.Lat,this.couverinfo.Lon);
     ...
     /* const options={
      ...
      /*this.couvertinfoList=this.loadCouverts();
      console.log(this.couvertinfoList[0])
     
      console.log(this.couvertinfoList.length)
      for (var i=0;i<this.couvertinfoList.length;i++){
          console.log("hello")
      }*/
  ...
  /*gotoMyPosition(){
    ...
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.loader.dismiss();
      var location=new google.maps.LatLng(this.couverinfo.Lat,this.couverinfo.Lon);
      this.map.setCenter(location);
      var marker= new google.maps.Marker({
        position: location,
        map: this.map,
